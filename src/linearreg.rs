...
impl BayesianLinearRegression {
    ...
    pub fn with_default(input_dim: usize, rff_features: usize) -> Self {
        BayesianLinearRegression::new(input_dim, rff_features, 1.0, 12345, 100.0, 1.0, 0.98, 1.0)
    }
    ...
    // Per-step online Bayesian update (exact linear-Gaussian within curriculum)
    pub fn update(&mut self, x_raw: &[f64], y: f64) -> f64 {
        let x = self.build_feature_vector(x_raw);
    ...
    pub fn start_new_curriculum(&mut self) {
        let inflation = 1.0 / self.lambda_prior; // lambda_prior <= 1.0
        if inflation > 1.0 + 1e-9 {
            for i in 0..self.v.len() {
                for j in 0..self.v[0].len() {
                    self.v[i][j] *= inflation;
                }
            }
        }
    }
