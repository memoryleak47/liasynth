...
fn valid_prod(
    ...
    let v: Box<[Node]> = a
        .iter()
        .map(|n| match n {
            GrammarTerm::NonTerminal(_, t) => Node::PlaceHolder(0, *t),
...
fn make_string(l: &SExpr, nonterms: &IndexMap<String, Ty>) -> String {
    match l {
        ...
        SExpr::List(xs) => format!(
            "({})",
            xs.iter().map(|x| make_string(x, nonterms)).join(" ")
        ),
    }
}
...
fn get_rst(
    ...
    match l {
        ...
        SExpr::List(xs) => xs
            .iter()
            .flat_map(|x| get_rst(x, nonterms, args))
            .collect::<Vec<GrammarTerm>>(),
    }
}
...
fn as_rule(
    ...
    refs: &mut IndexMap<usize, Vec<String>>,
) -> GrammarTerm {
    match s {
        SExpr::Ident(id) => {
            ...
            if let Ok(i) = id.parse::<Int>() {
                return GrammarTerm::ConstInt(i, Ty::NonTerminal(nt));
            }
            if id == "true" {
                return GrammarTerm::ConstBool(true, Ty::NonTerminal(nt));
            }
            if id == "false" {
                return GrammarTerm::ConstBool(false, Ty::NonTerminal(nt));
            }
            if args.contains_key(id) {
                return GrammarTerm::SynthArg(id.clone(), Ty::NonTerminal(nt));
            }

            panic!("unknown ident: {id}")
        }
        SExpr::List(l) => {
            let [SExpr::Ident(op), rst @ ..] = &l[..] else {
                panic!()
            };

            // special case for negative number constants:
            if let [SExpr::Ident(a)] = rst
                && op == "-"
            {
            ...
            let s = format!(
                "({})",
                l.iter().map(|x| make_string(x, nonterminals)).join(" ")
            );

            if defs.contains_key(op) {
                let rst = rst
                    .iter()
                    .map(|x| as_rule(nt, x, nonterminals, args, defs, refs))
                    .collect();
            ...
            let rst = l
                .iter()
                .flat_map(|x| get_rst(x, nonterminals, args))
                .collect::<Vec<_>>();
...
fn as_expr(
    ...
    synth_funs: &IndexMap<String, SynthFun>,
) -> Expr {
    match s {
        SExpr::Ident(id) => {
            if let Ok(i) = id.parse::<Int>() {
                return Expr::ConstInt(i);
            }
            if id == "true" {
        ...
        SExpr::List(l) => {
            ...
            if op == "let" {
                let [SExpr::List(bindings_), expr] = rst else {
                    panic!()
                };
                let mut bindings = IndexMap::new();
                for b in bindings_ {
                    let SExpr::List(b) = b else { panic!() };
                    let [SExpr::Ident(var), ex] = &**b else {
                        panic!()
                    };
                ...
                let rst = rst
                    .iter()
                    .map(|x| as_expr(x, defined_funs, synth_funs))
                    .collect();
...
fn handle_synth_fun(l: &[SExpr], synth: &mut SynthProblem) {
    let [
        ...
        SExpr::List(args_),
        SExpr::Ident(ret),
        SExpr::List(nonterminals_),
        SExpr::List(nonterminal_defs_),
    ] = l
    else {
        panic!()
    };
    ...
    for a in args_ {
        ...
        let [SExpr::Ident(l), SExpr::Ident(r)] = &v[..] else {
            panic!()
        };
    ...
    for a in nonterminals_ {
        let SExpr::List(v) = a else { panic!() };
        let [SExpr::Ident(name), SExpr::Ident(ty)] = &v[..] else {
            panic!()
        };
    ...
    for (nt, a) in nonterminal_defs_.iter().enumerate() {
        let SExpr::List(v) = a else { panic!() };
        let [SExpr::Ident(name), SExpr::Ident(ty), SExpr::List(rules)] = &v[..] else {
            panic!()
        };
        let prod_rules = rules
            .iter()
            .map(|x| {
                as_rule(
                    nt,
                    ...
                    &args,
                    &synth.defined_funs,
                    &mut nonterminal_refs,
                )
            })
            .collect();

        nonterminal_defs.insert(
            name.clone(),
            NonterminalDef {
                ty: as_ty(ty),
                prod_rules,
...
fn handle_declare_var(l: &[SExpr], synth: &mut SynthProblem) {
    let [SExpr::Ident(s), SExpr::Ident(ty)] = l else {
        panic!()
    };
...
fn handle_define_fun(l: &[SExpr], synth: &mut SynthProblem) {
    ...
    for a in args_ {
        let SExpr::List(v) = a else { panic!() };
        let [SExpr::Ident(var), SExpr::Ident(ty)] = &v[..] else {
            panic!()
        };
...
fn handle_constraint(l: &[SExpr], synth: &mut SynthProblem) {
    ...
    synth
        .constraints
        .push(as_expr(e, &synth.defined_funs, &synth.synthfuns));
}
