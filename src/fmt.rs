...
fn term_to_z3_impl(i: usize, t: &Term, vars: &[String]) -> String {
    ...
    match n {
        ...
        Node::VarInt(v, _) | Node::VarBool(v, _) => {
            vars.get(*v).cloned().unwrap_or_else(|| format!("v{v}"))
        }
        _ => {
...
fn fill_template(template: &str, args: &[String]) -> String {
    ...
    let mut out =
        String::with_capacity(template.len() + args.iter().map(|s| s.len()).sum::<usize>());
